...
num_files_moved=$(rsync --rsync-path=/bin/rsync -auvh --timeout=60 --remove-source-files --no-perms --stats --log-file=/tmp/archive-rsync-cmd.log "$CAM_MOUNT/TeslaCam/SentryClips/"* "$CAM_MOUNT/TeslaCam/SavedClips/"* "$user@$server:$path" | awk '/files transferred/{print $NF}')
